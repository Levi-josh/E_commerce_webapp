...
return (
        <div className={` ${items.length >= 1 ?'h-full pb-10':'h-screen'}  ${bgcolor} `}>
            <header className={`fixed flex sm:justify-between gap-4 ${bgcolor} ${textcolor}  $  sm:gap-0 items-center h-20 sm:h-24 md:h-24 lg:h-24 px-2 sm:px-5 md:px-2 lg:px-4 w-full m-0  z-10 shadow-lg `}>
                ...
                <div  className={`${showinput1 ? 'hidden' : 'block'} right-4  sm:right-6 md:right-4 lg:right-6 absolute hover:cursor-pointer bg-gray-200 p-2 rounded-lg`}><FaSearch onClick={searchfunc} /></div>
            </header >
                ...
                <div className={`sm:pt-28 pt-24     `}>
                    ...
                    <div className={`flex flex-col gap-3 lg:gap-4  w-full ${bgcolor}  justify-center mb-3 sticky top-20 sm:top-24  py-3  sm:py-4 `}>
                        ...
                        <div className='flex px-3 sm:justify-center justify-between gap-2 md:gap-12 lg:gap-20 sm:gap-10'>
                            <button className={`w-full font-semibold sm:w-16 lg:w-13 ${mode.colormode?'bg-stone-800':'bg-gray-200'} ${textcolor} py-1 md:py-2 text-black border hover:bg-yellow-900 hover:text-white focus:bg-yellow-900 focus:text-white  rounded-full `}>Clothes</button>
                            <button className={`w-full font-semibold sm:w-16 lg:w-13 ${mode.colormode?'bg-stone-800':'bg-gray-200'} ${textcolor}  border py-1 md:py-2 text-black hover:bg-yellow-900 hover:text-white focus:bg-yellow-900 focus:text-white  rounded-full`}>Shoes</button>
                            <button className={`w-full font-semibold sm:w-16 lg:w-13 ${mode.colormode?'bg-stone-800':'bg-gray-200'} ${textcolor} py-1 md:py-2 text-black border hover:bg-yellow-900 hover:text-white focus:bg-yellow-900 focus:text-white  rounded-full`}>Glasses</button>
                            <button className={`w-full font-semibold sm:w-16 lg:w-13 ${mode.colormode?'bg-stone-800':'bg-gray-200'} ${textcolor} py-1 md:py-2 text-black border  hover:bg-yellow-900 hover:text-white focus:bg-yellow-900 focus:text-white  rounded-full `}>Watches</button>
                        </div>
                    </div>
