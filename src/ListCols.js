...
import CartImage from './AdobeStock_518309079_Preview.png'
    ...
    <div className={`w-full overflow-auto h-screen   ${bgcolor} ${textcolor}`}>
        <header className={`fixed flex sm:justify-between gap-4 ${bgcolor} items-center h-20 sm:h-24 md:h-24 lg:h-24 px-3 lg:px-4 w-full m-0  z-10  `}>
          {showinput?<NavLink to='../' className={'sm:absolute '}> <FaAngleLeft className='font-bold  text-xl md:text-2xl ' ></FaAngleLeft></NavLink>:<FaAngleLeft className='font-bold  text-xl md:text-2xl hover:cursor-pointer  ' onClick={changeInput} ></FaAngleLeft>}
            ...
            {showinput&& <div onClick={changeInput} className='flex items-center text-sm sm:text-base absolute  right-3 text-white justify-center bg-brown px-4  sm:px-6 py-2 hover:cursor-pointer rounded-xl'>
            <p>Create cart</p>
            </div>}
        </header >
        <div className={' w-full h-full  pt-20 sm:pt-24 lg:pt-0 px-3 lg:items-start  lg:flex lg:justify-end lg:px-4'}>
          <div className='flex item-center  w-full lg:h-full lg:left-3 flex-col lg:fixed   top-24  lg:block lg:w-20   '>
            <div className={` w-full flex gap-3 md:gap-5 items-center md:items-start lg:items-center justify-start lg:h-175 p-6 sm:p-7 ${!mode.colormode?'bg-gray-200':'bg-stone-800'}  rounded-lg  flex-col lg:mt-10 lg:m-0  m-auto `}>
              ...
              <ul className='list-disc list-outside pl-2 sm:pl-3 space-y-2 sm:space-y-3'>
                <li className={'text-sm sm:text-base font-serif '}>Firstly,click on the "create cart" botton to create a cart.</li>
                <li className={'text-sm sm:text-base font-serif '}> Once the cart is created,check the input box to select the cart.</li>
                <li className={'text-sm sm:text-base font-serif '}> After following the above instructions,head back to add your items to your selected cart.</li>
              </ul>
            </div>
          </div>
