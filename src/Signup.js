  ...
  return (
    ...
    <div className={`w-full sm:w-140  lg:w-130  xl:w-140 lg:h-96  lg:rounded-xl overflow-hidden  justify-center lg:items-center   items-start flex flex-col lg:flex-row  ${window.matchMedia('(max-width: 600px)').matches?'':mode.colormode?'addShadow2':'addShadow'}  gap-10  lg:gap-12 `}>
        <div className='w-full md:h-full flex items-center justify-center hero before:bg-blurblack2  '>
          <img src='https://images.unsplash.com/photo-1558769132-cb1aea458c5e?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8Y2xvdGhlc3xlbnwwfHwwfHx8MA%3D%3D' className='h-full w-full lg:rounded-bl-xl lg:rounded-tl-xl  lg:rounded-t-none        bg-no-repeat bg-cover bg-center object-cover   ' />
          <div className=' text-white text-2xl  absolute  px-10 py-3 z-30 font-semibold sm:text-3xl rounded-lg '><p>Hi,Welcome</p></div>
        </div>
