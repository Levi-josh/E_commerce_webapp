        ...
        <div className='flex justify-center items-center w-full  h-screen '>
            ...
            <div className='w-130 sm:w-140  lg:w-130  xl:w-140 lg:h-96  rounded-xl   justify-center lg:items-center   items-start flex flex-col lg:flex-row shadow-2xl gap-3 sm:gap-6  lg:gap-12 '>
                ...
                <form  onSubmit={handleSubmit}className='flex flex-col gap-3 sm:gap-4 lg:gap-5 pb-3 sm:pb-6  w-full  lg:pr-12 lg:pl-0 px-3 sm:px-7 lg:px-6   '>
                    ...
                    <input type='text' placeholder='Username'  value={formData.input1} name="input1" onChange={handlechange} className='   outline-none  border border-yellow-900 py-1 pl-2 lg:py-2   text-black placeholder:text-neutral-300 placeholder:font-semibold  w-full' />
                    </div>
                    <div className='w-full relative h-14 flex items-end '>
                    {user.password&&<p className=' text-red-500 absolute top-0 text-sm sm:text-base'>{user.password}</p>}
                    <input type='password' placeholder='Password'  value={formData.input2} name="input2" onChange={handlechange}  className='  outline-none  border border-yellow-900 py-1 lg:py-2 pl-2  text-black placeholder:font-semibold placeholder:text-neutral-200 w-full  ' />
                    </div>
                    </div>
