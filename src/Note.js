...
const Note = () => {
    ...
    return (
        <div className={`  ${menubar ? 'home' : ''}${popout ? 'p-home' : ''} ${bgcolor} ${textcolor} h-screen `}>
            <header className={`fixed flex  items-center ${mode.colormode&&' outline outline-0.5 outline-yellow-950'} h-20 gap-4 sm:gap-0 sm:h-24 md:h-24 lg:h-24 px-3 lg:px-4  w-full m-0  z-10 ${menubar ? 'shadow-none lg:shadow-lg' : 'shadow-lg'} `}>
