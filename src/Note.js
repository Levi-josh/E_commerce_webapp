...
const Note = () => {
    ...
    return (
        <div className={`  ${menubar ? 'home' : ''}${popout ? 'p-home' : ''} h-full `}>
            <header className={`fixed flex  items-center h-20 sm:h-24 md:h-24 lg:h-24 px-3 lg:px-4  w-full m-0 bg-white z-10 ${menubar ? 'shadow-none lg:shadow-lg' : 'shadow-lg'} `}>
                <NavLink to='../' relative='path' className={'absolute '}> <FaAngleLeft className='font-bold  text-xl md:text-2xl ' ></FaAngleLeft></NavLink>
                <h1 className='m-auto font-semibold text-xl sm:text-2xl lg:text-3xl'> Notifications</h1>
                <FaSearch className='font-bold  text-xl md:text-2xl ' />
